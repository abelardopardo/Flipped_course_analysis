...
library(ggplot2)
library(plyr)
# strategy_names <- c('A', 'B1', 'B2', 'C1', 'C2', 'D', 'E', 'F')
...
create.trajectory.df <- function(data.in, traj_idx, new_idx) {
  ...
  # traj1 <- expand.grid(Week = week_names, Strategy = strategy_names, Prob = 0.125)

  ## Trajectory label
  traj_label <- paste('Trajectory', new_idx)
  
  ## Week 2
  result <- data.frame(Week = '2', Strategy = c('A', 'D', 'E', 'B1', 'C1'),
                       Trajectory = traj_label,
                       Prob = as.vector(data.in$cl.w2[traj_idx,]))

  ## Week 3
  result <- rbind(result,
                  data.frame(Week = '3', Strategy = c('A', 'E', 'D', 'B2'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w3[traj_idx,])),
                  data.frame(Week = '4', Strategy = c('E', 'C2', 'B1', 'A', 'B2'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w4[traj_idx,])),
                  data.frame(Week = '5', Strategy = c('D', 'B1', 'E', 'A'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w5[traj_idx,])),
                  data.frame(Week = '6', Strategy = c('C2', 'C1', 'E', 'D', 'A'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w6[traj_idx,])),
                  data.frame(Week = '7', Strategy = c('E', 'B1', 'D', 'C1'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w7[traj_idx,])),
                  data.frame(Week = '8', Strategy = c('B1', 'E', 'B2', 'A', 'D'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w8[traj_idx,])),
                  data.frame(Week = '9', Strategy = c('D', 'A', 'E', 'B1'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w9[traj_idx,])),
                  data.frame(Week = '10', Strategy = c('B1', 'A', 'D', 'B2', 'E'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w10[traj_idx,])),
                  data.frame(Week = '11', Strategy = c('D', 'B1', '-', 'E'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w11[traj_idx,])),
                  data.frame(Week = '12', Strategy = c('E', '-', 'B1', 'B2'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w12[traj_idx,])),
                  data.frame(Week = '13', Strategy = c('A', 'E', 'D', 'C1', 'C2'),
                             Trajectory = traj_label,
                             Prob = as.vector(data.in$cl.w13[traj_idx,])))

  ## Change to vector so that the sorting works as a string
  result$Strategy <- as.vector(result$Strategy)
  result$Trajectory <- as.vector(result$Trajectory)
  
  ## Sort the columns
  result <- result[with(result, order(Week, Strategy)),]
}
...
traj1 <- create.trajectory.df(lc6.fullcourse$probs, 1, 2)
traj2 <- create.trajectory.df(lc6.fullcourse$probs, 2, 5)
traj3 <- create.trajectory.df(lc6.fullcourse$probs, 3, 1)
...
#
prob_e <- c(
  sum(result[result$Trajectory == 'Trajectory 1' & result$Strategy == 'E',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 2' & result$Strategy == 'E',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 3' & result$Strategy == 'E',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 4' & result$Strategy == 'E',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 5' & result$Strategy == 'E',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 6' & result$Strategy == 'E',]$Prob))
prob_d <- c(
  sum(result[result$Trajectory == 'Trajectory 1' & result$Strategy == 'D',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 2' & result$Strategy == 'D',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 3' & result$Strategy == 'D',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 4' & result$Strategy == 'D',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 5' & result$Strategy == 'D',]$Prob),
  sum(result[result$Trajectory == 'Trajectory 6' & result$Strategy == 'D',]$Prob))

# Concatenate them to create the plot
result <- rbind(traj1, traj2, traj3, traj4, traj5, traj6)

ggplot(result, aes(x = Week, y = Prob, fill = Strategy)) +
...
ggsave('trajectories.tiff', 
       path = "~/Papers/1_Work/Pardo_Exploring_interactions",
