...
## it returns the input data frame with the new TIME_GAP column added  
compute.action.timegap <- function(trace.data) {
  trace.data$TIME_GAP <- vector(mode = "numeric", length = nrow(trace.data))
  trace.data$TIME_GAP[1] <- NA
  current.stud <- trace.data$STUDENT.ID[1];
  for(i in 2:nrow(trace.data)) {
    if (trace.data$STUDENT.ID[i]==current.stud) {
      trace.data$TIME_GAP[i] <- difftime(time1 = trace.data$TIMESTAMP[i], 
                                         time2 = trace.data$TIMESTAMP[i-1], 
                                         units = "secs" )
      ...
      if (trace.data$TIME_GAP[i] > 604800) 
        trace.data$TIME_GAP[i] <- NA
    } else {
      trace.data$TIME_GAP[i] <- NA  
      current.stud <- trace.data$STUDENT.ID[i]
    }  
  }
  trace.data
}
