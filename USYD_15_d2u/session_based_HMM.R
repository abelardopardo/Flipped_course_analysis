######################################################################################
## Create HMM for the entire course, as it was done for the 2014 dataset
## 
...
sorted.traces <- sorted.traces[,c(7,1,3,2,4,5,6,8,9)]
...
# just 2 students with less than 5 sessions
...
summary(session.count)  
boxplot(session.count)
# again, now 3 outliers; repeat the procedure
out.value <- boxplot.stats(session.count)$out
out.index <- which(session.count %in% out.value)
# remove the outlier (student)
stud.ids <- stud.ids[-out.index]
...
length(stud.ids)
# 348 (93.8% of the initial number of students (371))
...
# ========  ========  ========  =========
#        2  318191.7  319348.7  -158952.8
#        3  286122.8  287886.7  -142843.4
#        4  272693.4  275080.3  -136051.7 ***
#        5  273993.2  277019.3  -136622.6
#        6  253437.5  257119.0  -126263.7
#        7  250646.6  254999.7  -124785.3
# ========  ========  ========  =========

mod.fit.4s <- fit.hmm(final.features, ns = 4)
summary(mod.fit.4s)
...
write.csv(x = features.and.states[,c(1:9,11:15)], 
          file = "results/session_based_HMM_4_states.csv", 
          quote = F, row.names = F)
...
features.and.states <- read.csv(file = "results/session_based_HMM_4_states.csv")
...
saveRDS(stud.states.long, file = "Intermediate_results/student_4HMM_state_distribution_long.RData")

## find the maximum number of sessions per student
## required for setting the upper bound for Y-axis
max(rowSums(stud.states[,c(2:5)]))
# 167
...
ggplot(data=stud.states.long, aes(x=STUDENT_ID, y=freq, fill=state)) +
  ...
  ylab("Number of sessions (states)") + xlab("Students") +
...
## write this df to a file, it will be needed later for visualization
saveRDS(stud.states.perc.long, file = "Intermediate_results/student_4HMM_state_perc_dist_long.RData")
...
## get the data:
src.data <- read.csv(file = "results/session_based_HMM_4_states.csv")
str(src.data)
...
# 2.5th percentile is 15.675
# check the number of students with up to 15 sessions
which(session.count.df$SESSION_CNT <= 15)
# 9 of them
quantile(session.count.df$SESSION_CNT, probs = seq(0.9, 1, 0.025))
# 97.5th percentile is 154.975
which(session.count.df$SESSION_CNT >= 155)
# 9
## remove those with up to 15 sessions (2.5th perc.) or >= than 155 (97.5th perc)
## so, in total, 5% of students will be removed
stud.to.remove <- session.count.df$STUDENT_ID[(session.count.df$SESSION_CNT <= 15) | 
                                                (session.count.df$SESSION_CNT >= 155)]
# 18 in total
...
saveRDS(object = traminer.seq, 
        file = "Intermediate_results/state_sequence_data_SPS_format.RData")
## store the created sequences
saveRDS(object = equal.len.seq.df, 
        file = "Intermediate_results/state_sequence_data_df.RData")
...
traminer.seq <- readRDS(file = "Intermediate_results/state_sequence_data_SPS_format.RData")
...
state.trans.matrix <- matrix(data = c(0.530, 0.116, 0.173, 0.181,
                                      0.336, 0.344, 0.221, 0.100,
                                      0.245, 0.153, 0.356, 0.246,
                                      0.231, 0.122, 0.214, 0.433),
...
state.seq.df <- readRDS(file = "Intermediate_results/state_sequence_data_df.RData")
str(state.seq.df)
...
write.csv(student.clusters, file = "results/seq_of_HMM_states_student_clusters_indel=1.75.csv", 
          quote = F, row.names = F)
remove(state.seq.df)
...
student.clusters <- read.csv(file = "results/seq_of_HMM_states_student_clusters_indel2.csv")
...
summary(scores[,c(2,3)])
# 98 NAs; remove those
scores <- subset(scores, is.na(SC_FE_TOT)==F)

setdiff(student.clusters$STUDENT_ID, scores$user_id)
## one student is lacking scores; id:517
...
# chi-squared = 33.205, df = 2, p-value = 6.161e-08
## there is a sign. difference bewtween the clusters; check where the difference is
...
# chi-squared = 36.178, df = 2, p-value = 1.393e-08
...
## SKIP THIS PART - DIDN'T PROVE USEFUL

stud.state.cnt.long <- readRDS(file = "Intermediate_results/student_4HMM_state_distribution_long.RData")
...
stud.clusters <- read.csv(file = "results/seq_of_HMM_states_student_clusters_indel=1.75.csv")
str(stud.clusters)

state.seq.data <- read.csv(file = "results/session_based_HMM_4_states.csv")
str(state.seq.data)
...
clust.stats <- summary.stats(stud.states.clust[,c(2:6)], stud.states.clust$cl4, 4)
...
# only 1 (ST3_PERC) is normally distributed, but for consistency, use Kruskal-Wallis test for all the variables
